...
- Wave 9.3 — OpenAPI as code-of-record + spec/code drift CI gates (ADR 0050):
  - `TestArch_RouteHasSpecOperation` in `internal/identity/ports/route_spec_test.go` — bijective drift gate. AST-walks `mux.Handle("METHOD /api/v1/...", _)` literals + parses `api/openapi.yaml` paths; fails CI on any orphan in either direction. <50ms; joins `task test:arch`.
  - Wave 9.3a-align — added 27 operations + 18 schemas to `api/openapi.yaml` to close the existing drift (Wave 5 shipped 33 ops; code grew to 60; spec was 45% behind). `info.version` stays `0.2.0` (documenting existing routes ≠ semver bump).
  - `task ci:openapi` + `.spectral.yaml` — Spectral lint extending `spectral:oas` (Cloudflare/Box/DigitalOcean/Anthropic canon). Custom rules: every operation has summary+tags; paths start with `/api/v1/`; `/by-X/{X}` shape forbidden per ADR 0049 (grandfathered slug warns, doesn't error).
  - CI workflow audit — fixed silent gap: the `architecture` job ran arch tests against ONLY `integrationevents/...`, so Wave 6 (ADR 0047) + Wave 8 (ADR 0049) tests had never executed cloud-side. Workflow now runs all three arch-bearing packages, matching local `task test:arch`.
  - New `openapi` paths-filter output + `openapi-lint` cloud job. Paths-filter rules documented in ADR 0050 (cost-saving CI design; matches Kubernetes / Terraform / golang/go canon).
