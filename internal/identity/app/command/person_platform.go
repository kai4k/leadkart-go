...
// NewGlobalSuspendPersonHandler wires the handler.
func NewGlobalSuspendPersonHandler(p person.Repository) GlobalSuspendPersonHandler {
	if p == nil {
		panic("command: NewGlobalSuspendPersonHandler persons repository required")
	}
	return GlobalSuspendPersonHandler{persons: p}
}
...
func (h GlobalSuspendPersonHandler) Handle(ctx context.Context, cmd GlobalSuspendPersonCommand) error {
	...
	err := h.persons.UpdateByID(ctx, cmd.PersonID, func(p *person.Person) (bool, error) {
		if err := p.GloballySuspend(cmd.Reason); err != nil {
			return false, err
		}
		return true, nil
	})
...
// NewLiftPersonGlobalSuspensionHandler wires the handler.
func NewLiftPersonGlobalSuspensionHandler(p person.Repository) LiftPersonGlobalSuspensionHandler {
	if p == nil {
		panic("command: NewLiftPersonGlobalSuspensionHandler persons repository required")
	}
	return LiftPersonGlobalSuspensionHandler{persons: p}
}
...
// NewAnonymisePersonHandler wires the handler.
func NewAnonymisePersonHandler(p person.Repository) AnonymisePersonHandler {
	if p == nil {
		panic("command: NewAnonymisePersonHandler persons repository required")
	}
	return AnonymisePersonHandler{persons: p}
}
...
// NewUpdatePersonProfileHandler wires the handler.
func NewUpdatePersonProfileHandler(p person.Repository) UpdatePersonProfileHandler {
	if p == nil {
		panic("command: NewUpdatePersonProfileHandler persons repository required")
	}
	return UpdatePersonProfileHandler{persons: p}
}
...
func (h UpdatePersonProfileHandler) Handle(ctx context.Context, cmd UpdatePersonProfileCommand) error {
	...
	err := h.persons.UpdateByID(ctx, cmd.PersonID, func(p *person.Person) (bool, error) {
		if err := p.UpdateProfile(cmd.FirstName, cmd.LastName); err != nil {
			return false, err
		}
		return true, nil
	})
