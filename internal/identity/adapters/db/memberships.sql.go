...
type ListRoleAssignmentsByMembershipRow struct {
	MembershipID pgtype.UUID
	RoleID       pgtype.UUID
	TenantID     pgtype.UUID
	AssignedAt   pgtype.Timestamptz
...
func (q *Queries) ListRoleAssignmentsByMembership(ctx context.Context, membershipID pgtype.UUID) ([]ListRoleAssignmentsByMembershipRow, error) {
	rows, err := q.db.Query(ctx, listRoleAssignmentsByMembership, membershipID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListRoleAssignmentsByMembershipRow
	for rows.Next() {
		var i ListRoleAssignmentsByMembershipRow
		if err := rows.Scan(
			&i.MembershipID,
			&i.RoleID,
			&i.TenantID,
