...
| `TestArch_NoSlogDefault` | (none — retired in May 2026 slog-injection sweep) | Every production caller now injects `*slog.Logger` at construction; nil panics at NewX (Mat Ryer 2024 + Khorikov §6). |
...
| TD15 | `TestArch_IntegrationTestsHaveTimeout` | Every integration `Test*` wraps in `context.WithTimeout(t.Context(), N)` OR `arch-test:no-timeout-needed` | 22 sites; inventory + platform + crm adapters wrapped at 30s; cmd/api fixtures + messaging file-level annotated (existing internal timeouts in fixture). |
