...
// ErrInvalidMessage is returned by validation failures inside the
// gateway surface (Message construction, recipient checks). Distinct
// from [ErrInvalid] in email.go (Address VO validation) — same
// package; different concern. Wrapped per the standard go errors.Is
// convention.
var ErrInvalidMessage = errors.New("email: invalid message")

// ErrSend is returned when the underlying provider call fails.
// Distinct from ErrInvalidMessage so callers can decide whether to
// retry (transient) vs surface a 4xx (validation).
...
func NewMessage(to, from Address, subject, bodyText string, opts ...MessageOption) (Message, error) {
	if to.IsZero() {
		return Message{}, fmt.Errorf("%w: to address required", ErrInvalidMessage)
	}
	if from.IsZero() {
		return Message{}, fmt.Errorf("%w: from address required", ErrInvalidMessage)
	}
	if strings.TrimSpace(subject) == "" {
		return Message{}, fmt.Errorf("%w: subject required", ErrInvalidMessage)
	}
	if strings.TrimSpace(bodyText) == "" {
		return Message{}, fmt.Errorf("%w: bodyText required (HTML alone is not enough — fallback for non-HTML clients per RFC 2046)", ErrInvalidMessage)
	}
	m := Message{
		to:       to,
		from:     from,
...
// in `internal/common/email/{provider}/`; the composition root in
