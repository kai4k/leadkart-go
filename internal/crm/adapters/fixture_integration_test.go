...
// crm/adapters integration test package.
//
...
//   - For tests that NEED cross-tenant isolation (subscriber-driven
//     bus tests that share a Watermill topic across parallel runs):
//     call sharedPG.TruncateAll(t) at the top + DON'T call t.Parallel.
//
// Per-file factories (withTenantCtx, newSnapshot) stay in the files
// that own them — this fixture only owns the shared infrastructure
// (TestMain, sharedPG, crmRepoFixture).
...
// by TestMain; consumed by every test via crmRepoFixture /
// sharedPG.Pool().
//
...
// on the crm + identity schemas. Wraps m.Run() with goleak's
...
func TestMain(m *testing.M) {
	code := pgtest.RunMain(m, pgtest.Config{
		...
		Schemas: []string{"identity", "crm"},
		// DML on these.
		Grants: []string{"identity", "crm"},
...
// crmRepoFixture returns the shared package-scoped Postgres pool. Each
...
// withTenantCtx attaches a tenant ID to the context so the
// repository's tenant-scoped reads have a target tenant. Mirrors what
// the HTTP middleware does in production.
