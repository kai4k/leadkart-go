...
// "Event versioning". Module prefix MUST be `orders`.
var aliasRegex = regexp.MustCompile(`^orders\.[a-z][a-z0-9_-]*\.v\d+$`)

// TestArch_AllRegisteredEventsSatisfyMarker enforces every registered
// event implements TenantScoped. Orders has no Platform events.
...
// matches the `orders.{event-kebab}.v{N}` regex.
