...
// TestMain + shared pool for the dispatch/adapters integration package.
// One Postgres testcontainer + one shared pgxpool; per-test isolation via
// fresh tenant_id + RLS (tenant-scoped table). Mirrors the platform/crm
// fixture shape.
...
// leadkart_app role with grants on the dispatch schema, then wraps m.Run()
// with goleak's leak check.
func TestMain(m *testing.M) {
	code := pgtest.RunMain(m, pgtest.Config{
		Schemas: []string{"dispatch"},
		Grants:  []string{"dispatch"},
...
// dispatchPool returns the shared package-scoped pool; isolation comes from
