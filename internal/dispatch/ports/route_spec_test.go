...
// Dispatch HTTP route table and api/openapi.yaml per ADR 0050.
package ports_test
...
const (
	...
	dispatchPrefix = "/api/v1/dispatch/"
)
...
func TestArch_RouteHasSpecOperation(t *testing.T) {
	...
	t.Logf("Total: %d code routes, %d spec operations under %s*", len(codeSet), len(specSet), dispatchPrefix)
...
func extractCodeRoutes(path string) ([]routeKey, error) {
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		if !strings.HasPrefix(p, dispatchPrefix) {
			return true
		}
...
func extractSpecRoutes(path string) ([]routeKey, error) {
	...
	for specPath, ops := range spec.Paths {
		if !strings.HasPrefix(specPath, dispatchPrefix) {
			continue
		}
		for verb := range ops {
