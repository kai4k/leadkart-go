// route_registration_test.go — TestArch_ route-conflict gate per
// ADR 0049. Mirror of internal/tasks/ports/route_registration_test.go.
package ports_test
...
import (
	...
	"github.com/leadkart/leadkart-go/internal/dispatch/ports"
...
// TestArch_RouteRegistration_NoConflicts asserts Dispatch route
// registration completes without a Go 1.22 ServeMux pattern-overlap
// panic.
func TestArch_RouteRegistration_NoConflicts(t *testing.T) {
	...
	req, _ := http.NewRequestWithContext(t.Context(), http.MethodGet, "/api/v1/dispatch/consignment-notes", nil)
	_, pattern := mux.Handler(req)
	if pattern == "" {
		t.Fatal("expected at least one Dispatch route registered")
	}
}
