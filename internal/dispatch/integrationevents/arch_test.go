...
// "Event versioning". Module prefix MUST be `dispatch`.
var aliasRegex = regexp.MustCompile(`^dispatch\.[a-z][a-z0-9_-]*\.v\d+$`)

// TestArch_AllRegisteredEventsSatisfyMarker enforces every registered
// event implements TenantScoped. Dispatch has no Platform events.
...
// matches the `dispatch.{event-kebab}.v{N}` regex.
