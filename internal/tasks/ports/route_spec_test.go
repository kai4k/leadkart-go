...
// Tasks HTTP route table and api/openapi.yaml per ADR 0050.
package ports_test
...
func TestArch_RouteHasSpecOperation(t *testing.T) {
	...
	t.Logf("Total: %d code routes, %d spec operations under %s*", len(codeSet), len(specSet), tasksPrefix)
...
func extractCodeRoutes(path string) ([]routeKey, error) {
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		if !strings.HasPrefix(p, tasksPrefix) {
			return true
		}
...
func extractSpecRoutes(path string) ([]routeKey, error) {
	...
	for specPath, ops := range spec.Paths {
		if !strings.HasPrefix(specPath, tasksPrefix) {
			continue
		}
		for verb := range ops {
