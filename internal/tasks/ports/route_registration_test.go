...
// ADR 0049. Mirror of internal/crm/ports/route_registration_test.go.
package ports_test
...
// TestArch_RouteRegistration_NoConflicts asserts Tasks route
...
func TestArch_RouteRegistration_NoConflicts(t *testing.T) {
	...
	req, _ := http.NewRequestWithContext(t.Context(), http.MethodGet, "/api/v1/tasks/work-items", nil)
	_, pattern := mux.Handler(req)
	if pattern == "" {
		t.Fatal("expected at least one Tasks route registered")
	}
}
