...
--
-- Cross-cutting non-tenanted store; lives in the common
...
CREATE TABLE common.admin_impersonation_audit (
    id                uuid        PRIMARY KEY,
...
CREATE INDEX idx_admin_impersonation_audit_operator
    ON common.admin_impersonation_audit (operator_user_id, started_at_utc DESC);

CREATE INDEX idx_admin_impersonation_audit_target_tenant
    ON common.admin_impersonation_audit (target_tenant_id, started_at_utc DESC);

CREATE INDEX idx_admin_impersonation_audit_session
    ON common.admin_impersonation_audit (session_id)
    WHERE session_id IS NOT NULL;

COMMENT ON TABLE common.admin_impersonation_audit IS
...
DROP TABLE IF EXISTS common.admin_impersonation_audit;
